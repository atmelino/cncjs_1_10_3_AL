...
class GrblController {
  ...
  constructor(engine, options) {
    // // atmelino
    log.error("GrblController constructor");
    if (!engine) {
    ...
    this.runner.on("ok", (res) => {
      ...
      if (
        (this.workflow.state === WORKFLOW_STATE_PAUSED) && (received < sent)
      ) {
        if (!hold) {
  ...
  write(data, context) {
    ...
    const cmd = data.trim();
    this.actionMask.replyStatusReport = (cmd === "?") ||
      this.actionMask.replyStatusReport;
    this.actionMask.replyParserState = (cmd === "$G") ||
